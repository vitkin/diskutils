...
void docomp( const char *inp_filenames ) {
  ...
  if( numread1 - tocomp > 0 ) {
    ...
    if( fd12 >= 0 ) {
      ...
      next->prev = current; current = next;
      segmentcount++; imgfilepos += next->bytecount;
    }; filepos2 = filepos + difference;
  } else 
      if(diffcount==0) smry_printf( "files are the same.\n");
  smry_printf( "%lli blocks in common differ, %lli are the same.\n", diffcount, samecount );
...
int main( int argc, char *argv[] ) {
  ...
  while( argc > 0 ) {
    ...
    else if(!memcmp(arg,"if1=",3)) inp1_filename = arg+3;
    else if(!memcmp(arg,"if2=",3)) inp2_filename = arg+3;
